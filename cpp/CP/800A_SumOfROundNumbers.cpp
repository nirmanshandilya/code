...
(1≤n≤104). Represent the number n as a sum of round numbers using the minimum number of summands (addends).
...
(1≤t≤104) — the number of test cases in the input. Then t test cases follow.
...
int main()
{
    ...
    while (tc--)
    {
        int n, i = 0, copy, count = 0;
        cin >> n;
        copy = n;
        int flag[5] = {0};

        // enter digits into flag[]
        while (copy)
        {
            if (copy % 10)
            {
        ...
        // print number of non-zero terms
        cout << count << endl;
